...
export default function App() {
  ...
  const botaoRemover = () => {
    if (listaUsuarios.length > 0) {
      // Cria uma cópia da lista de usuários excluindo o último usuário
      const novaListaUsuarios = listaUsuarios.slice(0, -1);
  ...
  return (
    ...
    <View style={{ flex: 1, flexDirection: 'column', marginTop: 30, backgroundColor: '#FED2AF', alignContent: 'center'}}>
      ...
      <View style={{ justifyContent: 'center', flexDirection: 'row', alignItems: 'center', width: '70%', alignContent: 'center', marginLeft: 50, marginTop: 20 }}>
        <View style={{ marginRight: 20 }}>
          <Button title='Confirmar' color={'green'} onPress={botaoAdicionar}></Button>
        </View>
        <View style={{ marginRight: 20 }}>
          <Button title='Remover' color={'red'} onPress={botaoRemover}></Button>
        </View>
        <Button title='Limpar' color={'lightblue'} onPress={botaoLimpar}></Button>
      </View>
